...
export const getFieldValues = tool("clinicaltrials_get_field_values", {
    ...
    fieldStats: z
      .array(
        z.object({
          ...
          type: z
            .string()
            .describe("Field data type (ENUM, STRING, DATE, etc.)."),
          ...
          topValues: z
            .array(
              z.object({
                ...
                studiesCount: z
                  .number()
                  .describe("Number of studies with this value."),
              }),
            )
  ...
  format: (result) => {
    ...
    for (const stat of result.fieldStats) {
      lines.push(
        `**${stat.piece}** (${stat.uniqueValuesCount} unique values):`,
      );
      for (const tv of stat.topValues.slice(0, 15)) {
        lines.push(
          `  ${tv.value}: ${tv.studiesCount.toLocaleString()} studies`,
        );
      }
    }

...
describe('clinicaltrials_compare_studies tool', () => {
  ...
  // ========================================================================
  // Logic — Summary Analysis
  // ========================================================================

  describe('logic — summary analysis', () => {
    it('should detect common phases as a commonality', async () => {
      const study1 = makeStudy('NCT00000001');
      const study2 = makeStudy('NCT00000002');
      mockFetchStudy
        .mockResolvedValueOnce(study1)
      ...
      expect(result.summary.commonalities).toContainEqual(
        expect.stringContaining('PHASE3'),
      );
    });

    it('should detect different phases as a difference', async () => {
      const study1 = makeStudy('NCT00000001');
      const study2 = makeStudy('NCT00000002', {
        designModule: {
          studyType: 'INTERVENTIONAL',
      ...
      expect(result.summary.differences).toContainEqual(
        expect.stringContaining('different trial phases'),
      );
    });

    it('should detect common sponsors as a commonality', async () => {
      const study1 = makeStudy('NCT00000001');
      const study2 = makeStudy('NCT00000002');
      mockFetchStudy
        .mockResolvedValueOnce(study1)
      ...
      expect(result.summary.commonalities).toContainEqual(
        expect.stringContaining('Pharma Corp'),
      );
    });

    it('should detect different sponsors as a difference', async () => {
      const study1 = makeStudy('NCT00000001');
      const study2 = makeStudy('NCT00000002', {
        sponsorCollaboratorsModule: {
          leadSponsor: { name: 'Other Corp', class: 'INDUSTRY' },
          collaborators: [],
        },
      });
      mockFetchStudy
      ...
      expect(result.summary.differences).toContainEqual(
        expect.stringContaining('Different lead sponsors'),
      );
    });

    it('should detect common status as a commonality', async () => {
      const study1 = makeStudy('NCT00000001');
      const study2 = makeStudy('NCT00000002');
      mockFetchStudy
        .mockResolvedValueOnce(study1)
    ...
    it('should detect different statuses as a difference', async () => {
      const study1 = makeStudy('NCT00000001');
      const study2 = makeStudy('NCT00000002', {
        statusModule: {
      ...
      expect(result.summary.differences).toContainEqual(
        expect.stringContaining('Different statuses'),
      );
    });

    it('should detect geographic overlap as a commonality', async () => {
      const study1 = makeStudy('NCT00000001');
      const study2 = makeStudy('NCT00000002', {
        contactsLocationsModule: {
          locations: [
      ...
      expect(result.summary.commonalities).toContainEqual(
        expect.stringContaining('Common countries'),
      );
    });

    it('should not analyze fields that are not requested', async () => {
      const study1 = makeStudy('NCT00000001');
      const study2 = makeStudy('NCT00000002', {
        designModule: {
      ...
      // design differences should not appear because only 'status' was requested
      const allItems = [
        ...(result.summary.commonalities ?? []),
        ...(result.summary.differences ?? []),
      ];
      const hasDesignReference = allItems.some((item) =>
        item.includes('phase'),
      );
      expect(hasDesignReference).toBe(false);
    });

    it('should return undefined commonalities and differences when none found', async () => {
      // Studies with no design phases, no sponsors, no status, no locations
      const bareStudy = (nctId: string) => ({
        protocolSection: {
          identificationModule: { nctId, briefTitle: `Bare ${nctId}` },
        },
      });
      mockFetchStudy
        .mockResolvedValueOnce(bareStudy('NCT00000001'))
        .mockResolvedValueOnce(bareStudy('NCT00000002'));
      ...
      expect(result.summary.commonalities).toBeUndefined();
      expect(result.summary.differences).toBeUndefined();
    });
  });

...
```ts
const result = await csvParser.parse<Row>(csvString, {
  header: true,
  dynamicTyping: true,
});
...
interface DrawImageOptions {
  ...
  y: number;
  width?: number;
  height?: number; // default: intrinsic dimensions
  rotate?: number;
...
```ts
const { frontmatter, content, hasFrontmatter } =
  await frontmatterParser.parse<SkillMeta>(markdown);
if (hasFrontmatter) {

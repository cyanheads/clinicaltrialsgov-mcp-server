...
export const searchStudies = tool('clinicaltrials_search_studies', {
  ...
  input: z.object({
    ...
    nctIds: z
      ...
      .optional()
      .describe(
        'Filter to specific NCT IDs for batch lookups. Omit to search every study — an empty list is rejected, not treated as "no filter". Supplying this lifts the default unknown-enrollment exclusion, so an ID you name is never filtered out of its own lookup.',
      ),
    fields: z
      .array(z.string())
      .optional()
  ...
  async handler(input, ctx) {
    ...
    // registry, an empty `nctIds` list would drop `filter.ids` and answer an
    // ID-scoped question with that same whole registry, and an empty `fields`
    // list would silently fall back to the default projection. The constraint
    // strings fail both ways — '' is falsy and dropped, while ' ' is truthy and
    // forwarded, splicing a blank term into a joined boolean expression
    // (advancedFilter) or sending upstream a whitespace value it can only
    // reject (geoFilter, sort). Every list arm here judges `[]` in the handler
    // rather than at the schema: a schema-only rejection runs before the
    ...
    // so supplying nctIds lifts the default unknown-enrollment exclusion — the
    // rule getStudiesBatch and find_eligible already follow. Without it, an
    // explicit ID filter answered "no such study" for any study that publishes
    // no EnrollmentCount, while get_study_record returned it. Keyed on the
    // filter actually reaching upstream, matching buildSearchQuery's own test.
    const includeUnknownEnrollment = input.includeUnknownEnrollment || Boolean(filterIds?.length);
    ...
    // Reports the exclusion that actually ran, so it stays absent on the
    // ID-targeted path where the override lifted it.

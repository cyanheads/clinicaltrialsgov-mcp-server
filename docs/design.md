...
**Summary-mode adverse-event shape:** the counts (`groupCount`, `seriousEventCount`, `otherEventCount`), an `eventGroups` roster trimmed to `id` and `title`, and `topEvents` — up to 20 events ranked by the most participants affected in any one group, each with `term`, `organSystem`, `kind`, and `byGroup` rows `{ groupId, numAffected, numAtRisk }`. Nothing is summed across groups: groups can overlap (a crossover or second-course group re-counts participants of its parent arm), so a pooled figure is not an incidence (#137). Rows key on the id, as full mode does (#128), and each title prints once in the roster: repeated on every row, titles outweighed the counts (KEYNOTE-189's reached 99 characters and differed only in a trailing "(Second Course)").
...
| `locationLimit`  | `number?`  | Cap on the matched sites returned per candidate, 1–500. Default: `10`. A candidate whose matched sites are all closed carries one more — a recruiting site, the nearest one (in the requested country when one recruits there) when coordinates allow. |

**Output schema:**

| Field        | Type      | Description                                                                                                                                                                                                                                                                              |
| :----------- | :-------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `studies`    | `Study[]` | Matching studies, re-ranked and capped at `maxResults`. Each carries the requested eligibility fields: `nctId`, `briefTitle`, `briefSummary`, `overallStatus`, phase/enrollment/sponsor, `conditions`, interventions, `eligibilityModule` (age range, sex, healthy volunteers), a bounded `locations[]` — the sites matching the requested location at the narrowest level that matched (city, else state, else country), sorted by match score and capped at `locationLimit`, plus one recruiting site when none of the matched ones is recruiting (the nearest one by published coordinates, in the requested country when one recruits there, carrying `distanceMi`, else the first in match order) — and central contacts. A candidate whose sites were bounded also carries a top-level `locationSummary` (`totalLocations`, `matchedLocations`, `locationsTruncated`, `nearestRecruitingSiteAdded?`, `retrieveFullStudyWith`); it is absent when nothing was dropped, and `nearestRecruitingSiteAdded` is present only when that extra site was added, keeping its name when the site was picked by match order. |
...
5. Run the main search (`pageSize={maxResults}`, `fields=ELIGIBLE_FIELDS`, which includes `LocationGeoPoint`, `countTotal`) alongside two funnel-stage counts (condition-only, condition+location) in parallel
6. Re-rank studies so those whose own condition list names a requested condition surface above tangential MeSH-umbrella matches (#72/#79), then sort each study's locations by match to the patient location (city > state > country) and bound each list to the best-matching tier, capped at `locationLimit`. The tier is geographic, so a tier holding only closed sites would answer with a candidate nobody can enroll in — when that happens one recruiting site is admitted alongside it (#114): the one nearest the matched tier, by haversine distance from `LocationGeoPoint` to the closest site of the whole tier before the `locationLimit` cap, carrying `distanceMi`; ties keep match order. Candidates in the requested country are ranked alone whenever there are any, so a nearer site across a border (Seattle → Vancouver, BC) never displaces one at home; the rest of the tier score is ignored, since an out-of-state site is often the nearer one (Portland, OR → Vancouver, WA). When the matched tier or every recruiting candidate lacks coordinates, the first candidate in match order is admitted with no `distanceMi`, and the text makes no distance claim (#138). Coordinates are dropped from the returned sites once measured. The bound is applied here, once, so `structuredContent` and `format()` render the same sites (#46, #91)
